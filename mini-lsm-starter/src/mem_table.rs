...
impl MemTable {
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        ...
        .build();
        let entry = mem_iter.with_iter_mut(|iter| MemTableIterator::entry_to_item(iter.next()));
        mem_iter.with_mut(|x| *x.item = entry);
        mem_iter
    }
...
impl StorageIterator for MemTableIterator {
    ...
    /// 关键操作：
    /// with_iter_mut：可变借用迭代器并调用 next()
    /// entry_to_item：转换结果
    /// with_mut：更新 item 字段
    /// 原子性：通过自引用保证迭代器状态安全更新
    /// 错误处理：始终返回 Ok(())（实际实现中可能需处理错误）
    fn next(&mut self) -> Result<()> {
        let entry = self.with_iter_mut(|iter| MemTableIterator::entry_to_item(iter.next()));
        self.with_mut(|item| *item.item = entry);
        Ok(())
    }
}
