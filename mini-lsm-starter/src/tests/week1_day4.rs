...
fn test_sst_build_single_key() {
    let mut builder = SsTableBuilder::new(16, CompressionOptions::Snappy);
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"233"), b"233333");
...
fn test_sst_build_two_blocks() {
    let mut builder = SsTableBuilder::new(16, CompressionOptions::Snappy);
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"11"), b"11");
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"22"), b"22");
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"33"), b"11");
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"44"), b"22");
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"55"), b"11");
    builder.add(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"66"), b"22");
...
fn test_sst_seek_key() {
    ...
    for offset in 1..=5 {
        for i in 0..num_of_keys() {
            ...
            iter.seek_to_key(KeySlice::for_testing_from_slice_no_ts_no_ttl(
                &format!("key_{:03}", i * 5 + offset).into_bytes(),
            ))
            .unwrap();
        }
        iter.seek_to_key(KeySlice::for_testing_from_slice_no_ts_no_ttl(b"k"))
            .unwrap();
    }
}
